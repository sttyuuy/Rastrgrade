...
async function _hu(){
    ...
    if(!ss.uid){
        _lg('У предмета нет uid — обнови страницу','lose');
        return;
    }
    ...
    try {
        ...
        if(res.success){
            ...
            _sr({
                ...
                skin:null,
                value:'Потеряно: '+formatRastr(sv)+' '+_MF_ICON,
                canSell:false
            });
...
async function _ssk(sk){
    if(!_CU){_lg('Войди в аккаунт','lose');return;}
    if(!sk || !sk.uid){
        _lg('У этого предмета нет uid — обнови страницу','lose');
        return;
    }
    _ck();
    try {
