...
function _hu(){
    ...
    if(_a1(state.upgradeSource)>=_MX*0.99){_lg('Это максимальный скин','lose');_rz();return;}
    ...
    _pa(c,ok).then(function(){
        ...
        if(ok){
            ...
            _sr({type:'result-lose',icon:'X',title:'ПРОВАЛ',skin:null,value:'Потеряно: '+formatRastr(sv)+' '+_MF_ICON,canSell:false});
...
async function _withdraw(skin){
    ...
    if(!confirm('Вивести "'+skin.name+'" в Steam?')) return;
    try{
...
function _rinv(){
    ...
    sr.forEach(function(sk){
        ...
        e.innerHTML=renderSkinIcon(sk)+'<div class="name">'+sk.name+'</div><div class="price">'+formatRastr(_a1(sk))+' '+_MF_ICON+'</div><button class="sell-btn">Продать</button><button class="withdraw-btn">ВИВЕСТИ</button>';
...
function _ah(){
    ...
    var bcf=$('buyConfirm');if(bcf)bcf.addEventListener('click',function(){if(!_pp)return;var sk=_pp.skin;var up=_pp.price;var q=_pq||1;var tp=up*q;if(state.balance<tp){var mq=Math.floor(state.balance/up);if(mq<=0){_lg('Недостаточно средств','lose');_ck();return;}q=mq;tp=up*q;_lg('Хватило только на x'+q,'info');}state.balance-=tp;for(var i=0;i<q;i++)state.inventory.push(sk);state.purchases+=q;state.totalLost+=tp;state.profit-=tp;state.housePlayerLost+=tp;_xp(10*q);_by();_lg('Куплено: '+sk.name+' x'+q,'win');$('buyModal').classList.remove('show');_pp=null;_pq=1;_ui();_rsh();_rinv();_ri();save();});
