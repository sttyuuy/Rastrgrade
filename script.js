...
var _steamSelected={};
var _ndl=null;
var _MF='https://api.yrsproject.ru/public/image/Resize?shortname=metal.fragments&x=32&y=32';
var _MF_ICON='<img src="'+_MF+'" style="width:16px;height:16px;vertical-align:middle;display:inline-block" alt="⚙️">';
var _MF_ICON_BIG='<img src="'+_MF.replace('x=32&y=32','x=48&y=48')+'" style="width:24px;height:24px;vertical-align:middle;display:inline-block" alt="⚙️">';
...
function _ui(){
    ...
    DOM.balance.innerHTML=_MF_ICON+' '+formatRastr(state.balance);
    if(DOM.shopBalance)DOM.shopBalance.innerHTML=_MF_ICON+' '+formatRastr(state.balance);
    var p=DOM.profit;
    p.innerHTML=(state.profit>=0?'+':'')+_MF_ICON+' '+formatRastr(state.profit);
    p.className='hud-stat-value '+(state.profit>=0?'green':'red');
    DOM.invCount.textContent=state.inventory.length;
    DOM.invValue.innerHTML=_MF_ICON+' '+formatRastr(state.inventory.reduce(function(s,i){return s+i.price;},0));
    DOM.statTotalWon.innerHTML=_MF_ICON+' '+formatRastr(state.totalWon);
    DOM.statTotalLost.innerHTML=_MF_ICON+' '+formatRastr(state.totalLost);
    ...
    DOM.housePlayer.innerHTML=_MF_ICON+' '+formatRastr(state.housePlayerLost);
    DOM.houseCasino.innerHTML=_MF_ICON+' '+formatRastr(state.houseCasinoWon);
